...
PeerEntryPtrVector BitTorrentClient::getFastestToUpload(int infoHash) {
	...
	if (peerMap.size()) {
		PeerMapConstIt it = peerMap.begin();
		for (; it != peerMap.end(); ++it) {
			//            if (it->second.isInterested() && it->second.isUnchoked()) {
			//            if (it->second.isInterested()) {
...
void BitTorrentClient::addUnconnectedPeers(int infoHash, std::list<tuple<int,
		IPvXAddress, int> > & peers) {
...
//void BitTorrentClient::processCurrentThread(simtime_t processingTime) {
//    this->scheduleAt(simTime() + processingTime, &this->endOfProcessingTimer);
//}
...
void BitTorrentClient::removePeerInfo(int infoHash, int peerId, int connId,
	...
	// if the peer was activelly connected, decrement numOfActive, effectively
	...
	//        for (PeerMap::iterator it = peerMap.begin(); it != peerMap.end(); ++it) {
	//
	//        }
...
void BitTorrentClient::setInterested(bool interested, int infoHash, int peerId) {
...
void BitTorrentClient::setOldUnchoked(bool oldUnchoke, int infoHash, int peerId) {
	this->printDebugMsg("calling getPeerEntry from setOldUnchoked");
	this->getPeerEntry(infoHash, peerId).setOldUnchoked(oldUnchoke);
	//    throw std::logic_error("Implement this setOldUnchoked.");
}
//void BitTorrentClient::setUnchoked(bool unchoked, int infoHash, int peerId) {
//    this->getPeerEntry(infoHash, peerId).setUnchoked(unchoked, simTime());
//    throw std::logic_error("Implement this setUnchoked.");
//}
void BitTorrentClient::setSnubbed(bool snubbed, int infoHash, int peerId) {
	...
	//    throw std::logic_error("Implement this setSnubbed.");
}
	...
	if (!seeding) {
		...
		while (numActiveSlost && unconnectedList.size()) {
			...
			if (notConnected && notConnecting) {
				...
				this->activeConnectedPeers.insert(std::make_pair(infoHash,
						peerId));
			}
...
void BitTorrentClient::peerWireStatistics(cMessage const*msg, bool sending =
	...
	if (dynamic_cast<PeerWireMsg const*> (msg)) {
		...
		if (messageId == PW_PIECE_MSG) {
			content
					= static_cast<PieceMsg const*> (peerWireMsg)->getBlockSize();
			size -= content;
		}
...
void BitTorrentClient::removeThread(PeerWireThread *thread) {
	if (*this->threadInProcessingIt == thread) {
		...
		if (!this->allThreads.empty() && this->threadInProcessingIt
				!= this->allThreads.begin()) {
			--this->threadInProcessingIt;
		}
	} else {
...
void BitTorrentClient::initialize(int stage) {
	if (stage == 0) {
		...
		this->localIp = resolver.addressOf(
...
void BitTorrentClient::handleMessage(cMessage* msg) {
	if (!msg->isSelfMessage()) {
		...
		if (msg == &this->endOfProcessingTimer) {
			...
			assert(thread != NULL);
			//            if (msg->getKind() == BitTorrentClient::SELF_THREAD_DELETION) {
